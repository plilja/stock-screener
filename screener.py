...
def pick_cheap_relative_mean(data):
    ...
    d1 = data.apply(axis = 1, func = lambda x: x['pe'] >= 6 and x['pe'] < country_pe.get_group(x['land'])['pe'].quantile(0.25))
    data['rc1'] = d1.astype(int)
    # PE is positive and lower than average PE of companies in same sector
    d2 = data.apply(axis = 1, func = lambda x: x['pe'] >= 6 and x['pe'] < sector_pe.get_group((x['land'], x['branch']))['pe'].mean())
    data['rc2'] = d2.astype(int)
    # PS is lower than average of companies in same sector
    d3 = data.apply(axis = 1, func = lambda x: x['ps'] < sector_ps.get_group((x['land'], x['branch']))['ps'].mean())
...
def pick_quality_relative_mean(data):
    ...
    d1 = data.apply(axis = 1, func = lambda x: x['roa'] >= 0 and x['roa'] > country_roa.get_group(x['land'])['roa'].mean())
    data['rq1'] = d1.astype(int)
    # ROA is better than average ROA of sector
    d2 = data.apply(axis = 1, func = lambda x: x['roa'] >= 0 and x['roa'] > sector_roa.get_group((x['land'], x['branch']))['roa'].mean())
    data['rq2'] = d2.astype(int)
    # Earnings growth is in top 25 % of sector
    d3 = data.apply(axis = 1, func = lambda x: x['vinsttillväxt'] >= 0 and x['vinsttillväxt'] > sector_earnings_growth.get_group((x['land'], x['branch']))['vinsttillväxt'].quantile(0.75))
...
def pick_cheap(data):
    ...
    data['sum'] = data['sum'] + data['c1'] + data['c2'] + data['c3'] + data['c4'] + data['c5'] + data['c6'] + data['c7'] + data['c8'] + data['c9'] + data['c10']


def pick_quality(data):
    data['q1'] = (data['roe'] > 0.12).astype(int)
    data['q2'] = 2 * (data['roa'] > 0.08).astype(int)
    data['q3'] = (data['rörelsemarginal 5'] < data['rörelsemarginal']).astype(int)
    data['q4'] = (data['bruttomarginal 5'] < data['bruttomarginal']).astype(int)
